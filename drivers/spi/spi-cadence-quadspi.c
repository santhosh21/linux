				 ...
				 struct spi_mem *mem, struct phy_setting *phy)
{
	...
	dev_err(dev, "Unable to find RX low\n");
	return -ENOENT;
}
				  ...
				  u8 lowerbound)
{
	...
	dev_err(dev, "Unable to find RX high\n");
	return -ENOENT;
}
				   ...
				   struct spi_mem *mem)
{
	...
	dev_err(dev, "First tuning point: RX: %d TX: %d RD: %d\n", first.rx,
	...
	dev_err(dev, "Second tuning point: RX: %d TX: %d RD: %d\n", second.rx,
...
compare:
	...
	dev_err(dev, "Final tuning point: RX: %d TX: %d RD: %d\n", final.rx,
